...
public class CellFSM {
	...
	public CellFSM() {
		...
		myCells = new Cell[1][1];
		myCells[0][0] = new Cell();
	}
	...
	public void setImage(boolean[][] image) {
		myWidth = image.length;
		myHeight = image[0].length;
	...
	private void applyRule(Integer number) {
		switch (number) {
		case 2:
			initAutomata();
			break;
	...
	public Color[][] getColors() {
		Color[][] ar = new Color[myWidth][myHeight];
	...
	/**
	 * Now this method is unused.
	 * 
	 * @return Color[][] where rows and columns with ni marks are removed
	 */
	...
	private void initGreens() {
		...
		for (int i = 0; i < myWidth; i++) {
			for (int j = 0; j < myHeight; j++) {
				if (myCells[i][j].hasColor(CellColor.RED)) {
					/**
					 * борьба за несвязные рисунки
					 */
					// for (Cell cell : getCrossNeighbours(i, j)) {
					for (Cell cell : getNeighbours(i, j)) {
	...
	/**
	 * нужно 1. красить зеленым всю компоненту связности, а не слой; 2. если есть
	 * 2 смежные компоненты, красить обе.
	 * 
	 * на обеих проблемах вылетают восьмерки.
	 */
	...
	private void greenify(int i, int j) {
		if (myCells[i][j].hasColor(CellColor.RED)
				&& !myCells[i][j].hasColor(CellColor.GREEN)) {
	...
	private List<Cell> getNeighbours(int i, int j) {
		List<Cell> list = new ArrayList<Cell>(8);
		list.addAll(getCrossNeighbours(i, j));
		if (i != 0 && j != 0) {
			list.add(myCells[i - 1][j - 1]);
		}
		;
		if (i != myWidth - 1 && j != 0) {
			list.add(myCells[i + 1][j - 1]);
