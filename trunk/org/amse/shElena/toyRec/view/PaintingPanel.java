...
public class PaintingPanel extends JPanel {
	...
	public PaintingPanel() {
		...
		MouseInputListener listener = new MouseInputListener() {
			...
			public void mouseClicked(MouseEvent e) {
				if (myDrawingEnabled) {
					Point p = new Point(e.getPoint().x / myScale,
							e.getPoint().y / myScale);
					myPoints.add(p);
					myPoints.add(p);
					myPoints.add(null);
					repaint();
	...
	private void drawImage(Graphics g, BufferedImage image) {
		...
		if (scale == 0) {
			...
			int x = (panelW - scale * w) / 2;
			int y = (panelH - scale * h) / 2;

			g.drawImage(myImage, x, y, scale * w, scale * h, null);
		}
	}
